...
N'exécute pas `schema.sql` (il recrée les tables). Lance **dans l'ordre**,
une fois chacun, dans le SQL Editor :
1. `migration-securite.sql` — corrige la faille de rôle, ajoute la
   pré-attribution et l'accès sur invitation, sans perdre de données.
2. `migration-effacement.sql` — ajoute l'effacement des données de santé,
   la suppression de fiche et leur traçabilité.
...
## Conduire un entretien + minimiser les données de santé

Sur une fiche famille :

- **🎙️ Mode entretien** : assistant pas à pas, un volet à la fois, grands
  champs préremplis avec le dernier état — pensé pour dérouler l'entretien en
  visio façon questionnaire. « Terminer » enregistre tout d'un coup (historisé).
- **🖨️ Exporter en PDF** : imprime une synthèse propre (via l'impression du
  navigateur → « Enregistrer en PDF »). Sans nom de famille complet. Journalisé.
- **🧹 Effacer les données de santé** : supprime le contenu des 7 volets mais
  garde la fiche (code, prénom). Irréversible, confirmé, journalisé.
- **Supprimer la fiche** : supprime toute la famille (cascade).

**Flux recommandé sans hébergeur HDS** (données de santé hors appli le plus vite
possible) : `Mode entretien → Exporter en PDF` (rangé dans un espace sécurisé :
Drive pro, dossier papier) `→ Effacer les données de santé`. L'appli ne conserve
alors que le strict minimum non sensible.
...
| `migration-securite.sql` | Mise à jour sécurité d'une base **existante** (à lancer en 1er) |
| `migration-effacement.sql` | Effacement/export d'une base **existante** (à lancer en 2nd) |
